<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="类型字段" prop="类型">
         <el-input v-model="searchInfo.类型" placeholder="搜索条件" />
        ...
        <el-form-item label="客户系字段" prop="客户系">
         <el-input v-model="searchInfo.客户系" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="交付部字段" prop="交付部" width="120" />
        <el-table-column sortable align="left" label="类型字段" prop="类型" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="update项目预算-逐月收入Func(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await get项目预算-逐月收入List({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await delete项目预算-逐月收入ByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const update项目预算-逐月收入Func = async(row) => {
    const res = await find项目预算-逐月收入({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.re项目预算
...
// 删除行
const delete项目预算-逐月收入Func = async (row) => {
    const res = await delete项目预算-逐月收入({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await create项目预算-逐月收入(formData.value)
                  break
                case 'update':
                  res = await update项目预算-逐月收入(formData.value)
                  break
                default:
                  res = await create项目预算-逐月收入(formData.value)
                  break
